...
char *libxl_mcd(libxl_ctx *ctx, uint32_t domid, int is_detail)
{
    int rc;
    char _buf[32768];

    rc = xc_mcd_display_stats(ctx->xch, domid, _buf, 32768, is_detail);
    if (rc < 0) {
        LIBXL__LOG_ERRNOVAL(ctx, LIBXL__LOG_ERROR, rc,
            "Can not get mcd list");
        return NULL;
    }
...
char *libxl_mcd_memsize(libxl_ctx *ctx, int memsize)
{
    int rc;
    char _buf[32768];

    rc = xc_mcd_memsize(ctx->xch, _buf, 32768, memsize);
    if (rc < 0) {
        LIBXL__LOG_ERRNOVAL(ctx, LIBXL__LOG_ERROR, rc,
            "Can not get set memsize");
        return NULL;
    }
...
char *libxl_mcd_weight(libxl_ctx *ctx, uint32_t domid, uint32_t weight)
{
    int rc;
    char _buf[32768];

    rc = xc_mcd_weight(ctx->xch, domid, _buf, 32768, weight);
    if (rc < 0) {
        LIBXL__LOG_ERRNOVAL(ctx, LIBXL__LOG_ERROR, rc,
            "Can not set weight");
        return NULL;
...
char *libxl_mcd_cache_mode(libxl_ctx *ctx, uint32_t which, uint32_t cache_mode)
{
    int rc;
    char _buf[32768];

    rc = xc_mcd_cache_mode(ctx->xch, _buf, 32768, which, cache_mode);
    if (rc < 0) {
        LIBXL__LOG_ERRNOVAL(ctx, LIBXL__LOG_ERROR, rc,
            "Can not set cache_mode");
        return NULL;
    }
...
char *libxl_mcd_param(libxl_ctx *ctx, uint32_t which, uint32_t mode, uint32_t param)
{
    int rc;
    char _buf[32768];

    rc = xc_mcd_param(ctx->xch, _buf, 32768, which, mode, param);
    if (rc < 0) {
        LIBXL__LOG_ERRNOVAL(ctx, LIBXL__LOG_ERROR, rc,
            "Can not set cache_mode");
        return NULL;
