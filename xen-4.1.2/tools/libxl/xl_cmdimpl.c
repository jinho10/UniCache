...
int main_mcd_display(int argc, char **argv)
{
    const char *dom = NULL;
    char *buf = NULL;
    int all = 0;
    ...
    while ((opt = getopt(argc, argv, "dah")) != -1) {
        switch (opt) {
        case 'd':
            is_detail = 1;
            break;
    ...
    //buf = libxl_tmem_list(&ctx, domid, use_long);
    buf = libxl_mcd(&ctx, domid, is_detail);
    if (buf == NULL)
        return -1;
...
int main_mcd_memsize(int argc, char **argv)
{
    char *buf = NULL;
    int opt;
    int memsize = 0;
    ...
    buf = libxl_mcd_memsize(&ctx, memsize);
    if (buf == NULL)
        return -1;
...
int main_mcd_weight(int argc, char **argv)
{
    //const char *dom = NULL;
    char *buf = NULL;
    int opt;
    ...
    while ((opt = getopt(argc, argv, "hd:w:")) != -1) {
        switch (opt) {
        case 'w':
            weight = (uint32_t)atoi(optarg);
            break;
        case 'd':
            input_domid = (uint32_t)atoi(optarg);
            break;
        case 'h':
            help("mcd-weight");
            return 0;
    ...
    */
    ...
    if ( input_domid < 0 ) {
        fprintf(stderr, "You must specify a domain id (-d).\n\n");
        return 1;
    }

    //fprintf(stderr, "input_domid = %d, weight = %d\n", input_domid, weight);

    buf = libxl_mcd_weight(&ctx, input_domid, weight);
    if (buf == NULL)
        return -1;
...
int main_mcd_cache_mode(int argc, char **argv)
{
    //const char *dom = NULL;
    char *buf = NULL;
    int opt;
    ...
    while ((opt = getopt(argc, argv, "hc:r:p:")) != -1) {
        switch (opt) {
        case 'c':
            which = 0;
            cache_mode = (uint32_t)atoi(optarg);
            break;
        case 'r':
            which = 1;
            cache_mode = (uint32_t)atoi(optarg);
    ...
    buf = libxl_mcd_cache_mode(&ctx, which, cache_mode);
    if (buf == NULL)
        return -1;
...
int main_mcd_param(int argc, char **argv)
{
    //const char *dom = NULL;
    char *buf = NULL;
    int opt;
    ...
    while ((opt = getopt(argc, argv, "hza:b:c:d:x:")) != -1) {
        switch (opt) {
        case 'a':
            which = 0;
            mode = 0;
            param = (uint32_t)atoi(optarg);
            break;
        case 'b':
            which = 0;
            mode = 1;
    ...
    buf = libxl_mcd_param(&ctx, which, mode, param);
    if (buf == NULL)
        return -1;
